...
function BookingStatRoomTwo ({ idBooking, dateBooking, timeBooking, user, getUrlPay }) {
  ...
  useEffect(() => {
    axios.get(`api/select-list/show?product_id=${idBooking}&show_date=${dateBooking}&show_time=${timeBooking}&room_id=2`).then((res) => {
  ...
  const getInfoBooking = (status, id, show_id, seat_row, seat_column, type) => {
    ...
    if (status) {
      ...
      if (check) {
        const updateShowRoom = [...show_room, {id, room_id: 2, show_id, seat_column, seat_row, condition: "empty", type}]
        ...
        />
        {
          current.type === "vip" ? (
            current.condition === "0" ? (
              ...
              >
                {(current.id - temp) <= 5 ? "A" + (current.id - temp) : (current.id - temp) <= 10 ? "B" + (current.id - temp - 5) : (current.id - temp) <= 15 ? "C" + (current.id - temp - 10) : (current.id -temp) <= 20 ? "D" + (current.id - temp - 15) : "E" + (current.id -temp -20)}
              </label>
            ) : (
              <label
  ...
  return (
    ...
    <div className="container my-4 p-0 border border-secondary rounded">
      <div className="header">
        <h3 className="title booking-stats-title m-3">Chọn ghế - Phòng 2</h3>
      </div>
