...
function BookingStatRoomTwo ({ idBooking, dateBooking, timeBooking, user }) {
  ...
  const getInfoBooking = (status, id, seat_row, seat_column, type) => {
    ...
    if (status) {
      ...
      if (check) {
        const updateShowRoom = [...show_room, {id, room_id: 2, show_id: idBooking, seat_column, seat_row, condition: "empty", type}]
