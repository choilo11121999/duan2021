...
function BookingStatRoomThree ({ idBooking, dateBooking, timeBooking, user }) {
  ...
  const getInfoBooking = (status, id, seat_row, seat_column, type) => {
    ...
    if (status) {
      ...
      if (check) {
        const updateShowRoom = [...show_room, {id, room_id: 3, show_id: idBooking, seat_column, seat_row, condition: "empty", type}]
    ...
    <>
      <div className="container my-4 p-4 border border-secondary rounded">
        <div className="header">
          <h5 className="title booking-stats-title">Chọn ghế - Phòng 3</h5>
        </div>
